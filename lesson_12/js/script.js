window.addEventListener('DOMContentLoaded', () => { // Скрипт будет выполняться только после того, как прогрузиться DOM дерево (HTML)
    ...
    forms.forEach(function(item) { // прописываем forEach, чтобы это касалось всех полей форм, цикл перебирает каждую форму
        item.addEventListener('submit', function(event) { // событие 'submit' нужно вешать именно на форму, а не на кнопку
            event.preventDefault(); // отменяем стандартное поведение браузера
            item.appendChild(statusMessage); // при отправке добавляем в форму div, в котором будет выводиться сообщение о статусе запроса

            let request = new XMLHttpRequest();
            request.open('POST', 'server.php');
            // request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); // если я хочу отправить данные через json
            ...
            let formData = new FormData(item); // указываем именно ту форму, откуда и получаем данные

            // let obj = {}; // этими строчками кода, мы превращаем наши данные полученные из формы в обычный читаемый объект
            // formData.forEach(function(value, key) {
            //     obj[key] = value;
            // });
            // let json = JSON.stringify(obj); // превратили обычный js объект в JSON

            // у POST запросов есть body, то есть то что он отправляет
            request.send(formData);
            // request.send(json); // если мы хотим открыть соединение с помощью json файла
